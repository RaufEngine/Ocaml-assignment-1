...
let rotate l n =
  ...
  let n = n mod len in
  let rec split_at k lst =
    if k = 0 then ([], lst)
    else match lst with
         | [] -> ([], [])
         | x :: xs -> let (left, right) = split_at (k - 1) xs in (x :: left, right)
  in
  let (left, right) = split_at (len - n) l in
  right @ left
...
let jump lst1 lst2 =
  let rec helper lst1 lst2 idx =
    match (lst1, lst2) with
    | ([], _) | (_, []) -> [] 
    | (x1 :: xs1, x2 :: xs2) ->
        if idx mod 2 = 0 then
          x2 :: helper xs1 xs2 (idx + 1) 
        else
          x1 :: helper xs1 xs2 (idx + 1) 
  in
  helper lst1 lst2 0 
...
let nth l n =
  let rec helper lst idx =
    match lst with
    | [] -> []
    | x :: xs -> if idx mod n = 0 then x :: helper xs 1 else helper xs (idx + 1)
  in
  helper l 1
...
let additivePersistence n =
  let rec helper count num =
    if num < 10 then count 
    else
      let sum = List.fold_left ( + ) 0 (digitsOfInt num) in 
      helper (count + 1) sum 
  in
  helper 0 n 
